...
public class CartServiceImpl implements ICartService {
	...
	public void delete(Integer[] cids, Integer uid) {
		...
		List<CartVO> list = getVOByCids(cids,uid);
		if(list.size()==0){
			return;
		}
		//通过以上查询结果得到有效的，新的cids数组
		Integer[] newcids = new Integer[list.size()];
		for(int i = 0;i<list.size();i++){
			newcids[i]=list.get(i).getCid();
		}
		//根据新的cids调用出就成对象删除购物车数据
	...
	private void deleteByCids(Integer[] cids){
		if(cids.length<1){
			throw new DeleteException("删除数据有误，请联系管理员");
		}
	}
